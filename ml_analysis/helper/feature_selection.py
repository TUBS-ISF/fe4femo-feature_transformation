...
def objective_function_zoo(model, X, y, no_use_X, no_use_y, inner_cv):
    y = y.iloc[:, 0]
...
def get_feature_selection(features : str, isClassification : bool, X_train : pd.DataFrame, y_train : pd.Series, X_test : pd.DataFrame, selector_args, estimator, group_dict : dict[str, list[str]], parallelism : int = 1, ):
    ...
    match features:
        ...
        case "SFS-forward":
            selector = SequentialFeatureSelector(estimator, cv=inner_cv, n_features_to_select=selector_args["n_features_to_select"], tol=selector_args["tol"], direction="forward", n_jobs=parallelism)
            selector.fit(X_train, y_train)
            return selector.transform(X_train), selector.transform(X_test)
        case "SFS-backward":
            selector = SequentialFeatureSelector(estimator, cv=inner_cv, n_features_to_select=selector_args["n_features_to_select"], tol=selector_args["tol"], direction="backward", n_jobs=parallelism)
            selector.fit(X_train, y_train)
            return selector.transform(X_train), selector.transform(X_test)
        case "harris-hawks":
            X_copy = pd.DataFrame(X_train)
            y_copy = pd.DataFrame(y_train)
            selector = HarrisHawkOptimization(partial(objective_function_zoo, inner_cv=inner_cv), **selector_args, minimize=False, n_jobs=parallelism)
            selected_feature_names = selector.fit(estimator, X_copy, y_copy, X_copy, y_copy, verbose=False)
            return X_train[selected_feature_names], X_test[selected_feature_names]
        case "genetic":
            X_copy = pd.DataFrame(X_train)
            y_copy = pd.DataFrame(y_train)
            selector = GeneticOptimization(partial(objective_function_zoo, inner_cv=inner_cv), **selector_args, minimize=False, n_jobs=parallelism)
            selected_feature_names = selector.fit(estimator, X_copy, y_copy, X_copy, y_copy, verbose=False)
            return X_train[selected_feature_names], X_test[selected_feature_names]
        case "HFMOEA":
        ...
        case "NDFS":
            ...
            return X_train.loc[:, sliced], X_test.loc[:, sliced]
...
def get_selection_HPO_space(features : str, trial : Trial, isClassification : bool, group_dict : dict[str, list[str]], no_features : int = 0) -> dict[str, Any]:
    ...
    match features:
        ...
        case "SFS-forward", "SFS-backward":
            # todo consider adding handling for tol-based selection
            return {
                "n_features_to_select" : trial.suggest_int("n_features_to_select", min_features, no_features),
                "tol" : None
            }
        ...
        case "genetic":
            return {
                "method": trial.suggest_categorical("method", [1, 2]),
                "population_size": trial.suggest_int("population_size", 10, 80),
                "n_iteration": 50  # todo n_iterations?
            }
        case "HFMOEA":
        ...
        case "embedded-tree":
            return {
                ...
                "max_features" : trial.suggest_int("max_features", min_features, no_features),
            }
        case "SVD-entropy":
            return {}
        case "NDFS":
