    ...
    with (SLURMMemRunner(scheduler_file=str(scheduler_path)+"/scheduler-{job_id}.json",
        ...
        with Client(runner) as client:
            with performance_report(filename=run_config["path_output"] + "/" + run_config["name"] + ".html"):
                ...
                n_jobs = math.ceil((int(os.getenv("SLURM_NTASKS", 7)) - 2) / 6) #2 less than tasks for scheduler and main-node
                ...
                X_train, X_test = get_feature_selection(precomputed.result(), features, is_classification, selector_config, model_instance_selector, feature_groups, parallelism=cores)
                end_FS = time.time()
                model_instance = get_model(model, is_classification, cores, model_config)
