...
path = base / "rq2_preprocessing_relative_dataset.csv"
out_path_full = base / "rq2_transformation_preprocessing_relative_full.pdf"
out_path_clipped = base / "rq2_transformation_preprocessing_relative_clipped.pdf"
...
transformation_order = (
    df.groupby("transformation_label")["delta_preprocessing"].median().sort_values().index.tolist()
)
...
def make_plot(clipped: bool, out_path: Path) -> None:
    ...
    plot.set(
        xlabel="Relative Preprocessing Runtime vs. None [s] (Transformation - None)",
