...
def build_relative_dataset(df_matched: pd.DataFrame) -> pd.DataFrame:
    ...
    for transformation in transformation_cols:
        ...
        rows["transformation_quality"] = df_matched[transformation]
        rows["baseline_quality"] = df_matched[BASELINE_COL]
        rows["delta_quality"] = rows["transformation_quality"] - rows["baseline_quality"]
...
def build_summary(df_relative: pd.DataFrame) -> pd.DataFrame:
    summary = (
        df_relative.groupby("transformation")["delta_quality"]
...
if __name__ == "__main__":
    ...
    in_path = base / "rq1_quality_matched_dataset.csv"
    relative_path = base / "rq1_quality_relative_dataset.csv"
    summary_path = base / "rq1_quality_relative_summary.csv"
