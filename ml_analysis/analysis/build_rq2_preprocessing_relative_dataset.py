...
def build_relative_dataset(df_matched: pd.DataFrame) -> pd.DataFrame:
    ...
    for transformation in transformation_cols:
        ...
        rows["preprocessing_value"] = df_matched[transformation]
        rows["baseline_preprocessing_value"] = df_matched[BASELINE_COL]
        rows["delta_preprocessing"] = rows["preprocessing_value"] - rows["baseline_preprocessing_value"]
        relative_rows.append(rows)

    return pd.concat(relative_rows, ignore_index=True)


def build_summary(df_relative: pd.DataFrame) -> pd.DataFrame:
    summary = (
        df_relative.groupby("transformation")["delta_preprocessing"]
...
if __name__ == "__main__":
    ...
    in_path = base / "rq2_preprocessing_matched_dataset.csv"
    relative_path = base / "rq2_preprocessing_relative_dataset.csv"
    summary_path = base / "rq2_preprocessing_relative_summary.csv"

    df_matched = pd.read_csv(in_path)
    relative = build_relative_dataset(df_matched)
    summary = build_summary(relative)
