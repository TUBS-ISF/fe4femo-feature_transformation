...
mkdir -p $HOME/feature_metrics/performance/out/$2/
...
while [[ maxIt -ge 1 ]] ; do
    ...
    output=$(sbatch --output=$HOME/feature_metrics/performance/out/$2/${maxIt}.out --job-name=${2}_${maxIt} --export=ALL,MODELNO=$maxIt --partition=${partition} $1)
